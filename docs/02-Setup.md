...
| DBB_MODELER_WORK  | The working directory for the DBB Git Migration Modeler on z/OS Unix System Services, where all the migration activities will be performed. Requires to be sized properly while it will store the entire copy of all application programs. | The default value is computed based on the home of the DBB Git Migration Modeler, to which a `-work` suffix is appended: `$DBB_MODELER_HOME-work` |
| DBB_MODELER_APPCONFIG_DIR  | The working folder where the initial version of the Application Descriptor files and the generated DBB Migration Mapping files are stored during the migration process. | The default value is computed based on the DBB Git Migration Modeler working folder, where a `work/migration-configuration` subfolder is created: `$DBB_MODELER_WORK/work/migration-configuration` |
| DBB_MODELER_APPLICATION_DIR  | The working folder where the DBB Git Migration Modeler will create directories for the repositories. | The default value is computed based on the DBB Git Migration Modeler working folder, where a `repositories` subfolder is created: `$DBB_MODELER_WORK/repositories` |
| DBB_MODELER_LOGS  | The working folder where the DBB Git Migration Modeler will store the log files for the various steps of migration process. | The default value is computed based on the DBB Git Migration Modeler working folder, where a `logs` subfolder is created: `$DBB_MODELER_WORK/logs` | 
...
| PUBLISH_ARTIFACTS | The flag to indicate if baseline packages should be uploaded to an Artifact Repository server (set to `true`) or not (set to `false`). | `false` |
