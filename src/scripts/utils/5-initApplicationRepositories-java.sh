...
if [ $rc -eq 0 ]; then
	...
	do
		...
		if [ "$APPLICATION_FILTER" == ",," ] || [[ ${APPLICATION_FILTER} == *",${applicationDir},"* ]]; then
			...
			CMD="java -cp \"$CLASSPATH\" com.ibm.dbb.migration.InitApplicationRepository -c \"$DBB_GIT_MIGRATION_MODELER_CONFIG_FILE\" -a \"$applicationDir\" -l \"$DBB_MODELER_LOGS/5-$applicationDir-initApplicationRepository.log\""
